 ...
 *  STATIC PROTOTYPES
 **********************/
static void style_mod_def(lv_style_t * style, lv_color_t highlight_color);
...
void lv_group_send_data(lv_group_t * group, uint32_t c)
{
    ...
    lv_res_t res_code = act->signal_func(act, LV_SIGNAL_CONTROLL, &c);
    if (res_code == LV_RES_ENT)
    {
        // Go into freeze mode to change value of current object
        lv_group_focus_freeze(group, true);
        (*group->obj_focus)->signal_func(*group->obj_focus, LV_SIGNAL_STYLE_CHG, NULL);
    } else if (res_code == LV_RES_REL)
    {
...
lv_style_t * lv_group_mod_style(lv_group_t * group, const lv_style_t * style)
{
    ...
    lv_color_t highlight_color = LV_COLOR_ORANGE;

    if (group->frozen == 1)
    {
        highlight_color = LV_COLOR_GREEN;
    }

    if(group->style_mod != NULL) group->style_mod(&group->style_tmp, highlight_color);
    else style_mod_def(&group->style_tmp, highlight_color);
...
static void style_mod_def(lv_style_t * style, lv_color_t highlight_color)
{
    ...
    style->body.border.color = highlight_color;

    /*If not empty or has border then emphasis the border*/
    if(style->body.empty == 0 || style->body.border.width != 0) style->body.border.width = LV_DPI / 20;

    style->body.main_color = lv_color_mix(style->body.main_color, highlight_color, LV_OPA_70);
    style->body.grad_color = lv_color_mix(style->body.grad_color, highlight_color, LV_OPA_70);
    style->body.shadow.color = lv_color_mix(style->body.shadow.color, highlight_color, LV_OPA_60);

    style->text.color = lv_color_mix(style->text.color, highlight_color, LV_OPA_70);
#else
    style->body.border.opa = LV_OPA_COVER;
